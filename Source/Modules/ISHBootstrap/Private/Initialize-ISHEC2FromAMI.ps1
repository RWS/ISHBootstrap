...
Function Initialize-ISHEC2FromAMI {
    ...
    process {
        ...
        $replacementMatrix = @(
            ...
            # basehostname
            @{
                CurrentValue = Get-ISHDeploymentParameters -Name basehostname -ValueOnly
                NewValue     = $HostName
            }
            # localservicehostname
            @{
                CurrentValue = Get-ISHDeploymentParameters -Name localservicehostname -ValueOnly
                NewValue     = $env:COMPUTERNAME
            }
            # machinename
            @{
                CurrentValue = Get-ISHDeploymentParameters -Name machinename -ValueOnly
                NewValue     = $env:COMPUTERNAME
            }
        ) | Where-Object {
        ...
        $packagesPath = Get-ISHPackageFolderPath
