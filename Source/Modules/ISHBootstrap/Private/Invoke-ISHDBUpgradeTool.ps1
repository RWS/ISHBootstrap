...
Function Invoke-ISHDBUpgradeTool {
    ...
    process {
        ...
        $inputParametersPath = Get-ISHInputParametersItem | Select-Object -ExpandProperty FullName
        ...
        switch ($PSCmdlet.ParameterSetName) {
            { $_ -like 'Upgrade*' } {
                ...
                $ishDBVersion = Get-ISHDatabaseVersion
        ...
        if ($pscmdlet.ShouldProcess("$dbUpgradeToolPath")) {
            ...
            if ($mustExecuteDBUpgradeTool) {
                ...
                if ($prp.IsInRole($adm)) {
                    ...
                    if ($failReason) {
                        Write-Warning "DBUpgradeTool.exe og file available in $dbUpgradeToolLogPath"
